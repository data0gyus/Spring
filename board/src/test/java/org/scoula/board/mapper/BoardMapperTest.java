...
class BoardMapperTest {
    ...
    @Test
    @DisplayName("BoardMapper의 목록불러오기")
    public void getList(){
    ...
    public void update() {
        BoardVO board = new BoardVO();
        board.setNo(6L);                    // 수정할 게시글 번호
        board.setTitle("수정된 제목");
        board.setContent("수정된 내용");
        board.setWriter("user00");

        int count = mapper.update(board);   // 수정된 행의 개수 반환
    ...
    public void delete() {
        int count = mapper.delete(6L);      // 삭제할 게시글 번호

        assertEquals(1, count);             // 1개 행이 삭제되었는지 확인
        log.info("DELETE COUNT: {}", count);

        // 삭제 후 조회 시 null 반환 확인
        BoardVO deletedBoard = mapper.get(6L);
